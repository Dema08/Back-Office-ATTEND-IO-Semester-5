...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h2 class="text-lg font-semibold">Edit Data Mata Kuliah</h2>
            <p class="text-xs text-slate-400">
                Ubah informasi mata kuliah: <span class="text-slate-200">{{ $mataKuliah->nama_matkul }}</span>
            </p>
        </div>
    ...
    <div class="rounded-2xl bg-slate-900/70 border border-slate-800/80 p-5 max-w-xl">
        <form action="{{ route('master.matakuliah.update', $mataKuliah->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
