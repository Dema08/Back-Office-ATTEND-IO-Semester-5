...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400">
                Ubah informasi kelas: <span class="text-slate-200">{{ $kelas->golongan }}</span>
            </p>
        </div>
    ...
    <div class="rounded-2xl bg-slate-900/70 border border-slate-800/80 p-5 max-w-xl">
        ...
        <form action="{{ route('master.kelas.update', ['kelas' => $kelas->id]) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="golongan" id="golongan"
                       value="{{ old('golongan', $kelas->golongan) }}" required
            ...
            <div>
                ...
                <select name="mata_kuliah_id" id="mata_kuliah_id" required
                    ...
                    @foreach ($mataKuliah as $mk)
                        <option value="{{ $mk->id }}"
                            @selected(old('mata_kuliah_id', $kelas->mata_kuliah_id) == $mk->id)>
            ...
            <div>
                ...
                <select name="dosen_id" id="dosen_id" required
                    ...
                    @foreach ($dosen as $d)
                        <option value="{{ $d->id }}"
                            @selected(old('dosen_id', $kelas->dosen_id) == $d->id)>
