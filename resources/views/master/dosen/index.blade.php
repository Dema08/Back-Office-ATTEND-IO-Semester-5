...
@section('page_title', 'Master Dosen')

@section('content')
    <div class="flex items-center justify-between mb-4">
        <div>
            <h2 class="text-lg font-semibold">Daftar Dosen</h2>
            <p class="text-xs text-slate-400">
                Kelola data dosen yang mengajar di sistem ATTEND-IO.
            </p>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('master.dosen.create') }}"
    ...
    @if ($dosen->isEmpty())
        <div class="rounded-2xl bg-slate-900/70 border border-slate-800/80 px-4 py-6 text-sm text-slate-400">
            Belum ada data dosen. Klik <span class="font-semibold text-slate-200">“Tambah Dosen”</span> untuk menambahkan.
        </div>
    @else
            ...
            <table class="min-w-full text-xs">
                <thead class="bg-slate-900/90 text-slate-400">
                    <tr class="border-b border-slate-800/80">
                        ...
                        <th class="px-4 py-2 text-left font-medium">NIP</th>
                        <th class="px-4 py-2 text-left font-medium">Nama Dosen</th>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 align-top">
                                <div class="flex flex-wrap items-center gap-2">
                                    <a href="{{ route('master.dosen.edit', $d->id) }}"
                                       class="text-xs text-indigo-400 hover:text-indigo-300">
                                        ✏️ Edit
                                    </a>

                                    <form action="{{ route('master.dosen.destroy', $d->id) }}"
                                          method="POST"
    ...
    <script>
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    text: `Apakah kamu yakin ingin menghapus dosen "${nama}"?`,
                    icon: 'warning',
